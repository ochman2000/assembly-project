...
Dane            SEGMENT
	...
	OrgProcZeg 		dd	?		;zmienna przechowywuj¹ca adres wybranej procedury
	poz				dw	0000h	;pozycja
...
NaszaProcZeg	PROC FAR
		...
		cmp poz, 160		; czy nie jest na koncu 
		jb porownaj
		...
		mov si, poz	
porownaj:
		cmp poz, 0		;jeœli poz=0, to nie mo¿emy czyœciæ poz-2
		je pisz

		
czysc:		mov BYTE PTR es:[si-2], ' '
		mov BYTE PTR es:[si-1], 00000000B


pisz:
		mov BYTE PTR es:[si], 5Ah	; 5Ah='Z' w tablicy ASCII
		mov BYTE PTR es:[si+1], 1Eh	; zolta litera na niebieskim tle
...
wyczysc_ekran: 
		...
		push cs		;push on stack the pointer to the segment containing the current program
		pop ds		;pop from stack the pointer to the segment where variables are defined
